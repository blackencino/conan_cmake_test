...
int main(int, char**)
{
    ...
    g_state = state.get();
    state->scene = std::make_unique<opengl_cuda_test::Scene>(opengl_cuda_test::make_scene());
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        opengl_cuda_test::draw(*(state->scene), glm::dvec2{static_cast<double>(display_w), 
